...
import (
	...
	"github.com/nholuongut/eksctl/pkg/git/executor"
)
...
var _ = Describe("git", func() {
	...
	Describe("RepoName", func() {
		It("can parse the repository name from a URL", func() {
			name, err := git.RepoName("git@github.com:nholuongut/eksctl.git")
			Expect(err).ToNot(HaveOccurred())
			Expect(name).To(Equal("eksctl"))

			name, err = git.RepoName("git@github.com:nholuongut/sock-shop.git")
			Expect(err).ToNot(HaveOccurred())
	...
	Describe("IsGitURL", func() {
		It("can determine if a string is a git URL", func() {
			Expect(git.IsGitURL("git@github.com:nholuongut/eksctl.git")).To(BeTrue())
			Expect(git.IsGitURL("https://github.com/nholuongut/eksctl.git")).To(BeTrue())
			Expect(git.IsGitURL("https://username@secr3t:my-repo.example.com:8080/nholuongut/eksctl.git")).To(BeTrue())
