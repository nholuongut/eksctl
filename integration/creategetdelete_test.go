...
import (
	...
	"github.com/nholuongut/eksctl/pkg/authconfigmap"
	"github.com/nholuongut/eksctl/pkg/ctl/cmdutils"
	"github.com/nholuongut/eksctl/pkg/eks"
	"github.com/nholuongut/eksctl/pkg/iam"
...
var _ = Describe("(Integration) Create, Get, Scale & Delete", func() {
	...
	Describe("when creating a cluster with 1 node", func() {
		...
		Context("and scale the initial nodegroup", func() {
			...
			It("{FLAKY: https://github.com/nholuongut/eksctl/issues/717} should make it 4 nodes total", func() {
		...
		Context("and add the second nodegroup", func() {
			...
			It("{FLAKY: https://github.com/nholuongut/eksctl/issues/717} should make it 8 nodes total", func() {
				test, err := newKubeTest()
				Expect(err).ShouldNot(HaveOccurred())
				defer test.Close()
		...
		Context("and scale the initial nodegroup back to 1 node", func() {
			...
			It("{FLAKY: https://github.com/nholuongut/eksctl/issues/717} should make it 1 nodes total", func() {
				test, err := newKubeTest()
				Expect(err).ShouldNot(HaveOccurred())
				defer test.Close()
		...
		Context("and deleting the cluster", func() {
			...
			It("{FLAKY: https://github.com/nholuongut/eksctl/issues/536} should not return an error", func() {
				if !doDelete {
			...
			It("{FLAKY: https://github.com/nholuongut/eksctl/issues/536} should have deleted the EKS cluster and both CloudFormation stacks", func() {
				if !doDelete {
					Skip("will not delete cluster " + clusterName)
				}
