...
version_pkg := github.com/nholuongut/eksctl/pkg/version

# The dependencies version should be bumped every time the build dependencies are updated
EKSCTL_DEPENDENCIES_IMAGE ?= nholuongut/eksctl-build:deps-0.15
EKSCTL_BUILDER_IMAGE ?= nholuongut/eksctl-builder:latest
EKSCTL_IMAGE ?= nholuongut/eksctl:latest
...
.PHONY: generate-all
# TODO: generate-ami is broken (see https://github.com/nholuongut/eksctl/issues/949 ), include it when fixed
